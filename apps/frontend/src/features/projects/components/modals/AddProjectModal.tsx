...
import { selectAddNewProjectOpen, projectsActions, createProject } from '../../store/slice';
...
export default function AddProjectModal() {
  ...
  const open = useAppSelector(selectAddNewProjectOpen) || false;
  ...
  const onSave = async () => {
    await dispatch<any>(createProject({ name, description, createdBy: ownerId, workspaceId }));
    dispatch(projectsActions.closeAddNew());
    setName(''); setDescription(''); setOwnerId(''); setWorkspaceId('');
  };

  return (
    <Dialog open={!!open} onClose={() => dispatch(projectsActions.closeAddNew())} fullWidth maxWidth="sm">
      ...
      <DialogActions>
        <Button onClick={() => dispatch(projectsActions.closeAddNew())}>Cancel</Button>
